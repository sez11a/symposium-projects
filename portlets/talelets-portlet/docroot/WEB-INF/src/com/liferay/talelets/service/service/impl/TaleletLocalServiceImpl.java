...
import com.liferay.talelets.social.TaleletActivityKeys;
 ...
 * </p>
 * 
 * Talelets are collaborative stories created by multiple users, defined by
 * three or more levels. The top level is a Tale, and it kicks off the story.
 ...
 * 
 * Since all three of these levels use the same entity to represent them, there
 * are rules for defining which is which.
 * 
 * Tale: A talelet with 0 in the parentId. Tale Starter: A talelet with a
 * populated parentId and a taleOrder of 1. Talelet: A talelet with a parent , a
 * taleOrder of > 1, and a taleletId which matches the primary key of its Tale
 * Starter.
 * 
 * @author Rich Sezov
...
public class TaleletLocalServiceImpl extends TaleletLocalServiceBaseImpl {
	...
	/**
	 * Gets Tales by descending date. In order to do this, a taleOrder of 0 is
	 ...
	 * @throws SystemException
	 */
	public List<Talelet> getTalesByDate(long groupId, int status, int start,
			int end) throws SystemException {

		List<Talelet> talelets = taleletPersistence.findByG_O(groupId, 0,
				status, start, end, new TaleletModifiedDateComparator());
	 ...
	 * @throws SystemException
	 */
	public List<Talelet> getTaleStartersByDate(long taleId, int status,
			int start, int end) throws SystemException {

		List<Talelet> talelets = taleletPersistence.findByTale(taleId, 1,
				status, start, end, new TaleletModifiedDateComparator());
	...
	public List<Talelet> getTalelets(long taleStarterId) throws SystemException {
		...
		List<Talelet> talelets = taleletPersistence
				.findByTaleStarter(taleStarterId);
	 ...
	 */
	public Talelet addTalelet(Talelet newTalelet, long userId,
			ServiceContext serviceContext) throws SystemException,
			PortalException {

		Talelet talelet = taleletPersistence.create(counterLocalService
				.increment(Talelet.class.getName()));
		...
		resourceLocalService.addResources(talelet.getCompanyId(),
				talelet.getGroupId(), userId, Talelet.class.getName(),
				talelet.getPrimaryKey(), false, true, true);

		long[] assetCategoryIds = serviceContext.getAssetCategoryIds();
		String[] assetTagNames = serviceContext.getAssetTagNames();

		assetEntryLocalService.updateEntry(userId, talelet.getGroupId(),
				Talelet.class.getName(), talelet.getTaleId(), assetCategoryIds,
				assetTagNames);

		socialActivityLocalService.addActivity(userId, talelet.getGroupId(),
				Talelet.class.getName(), talelet.getTaleId(),
				TaleletActivityKeys.ADD_TALELET, StringPool.BLANK, 0);

		// Workflow

		WorkflowHandlerRegistryUtil.startWorkflowInstance(
				talelet.getCompanyId(), talelet.getGroupId(), userId,
				Talelet.class.getName(), talelet.getTaleId(), talelet,
				serviceContext);

		return talelet;
	}

	public Talelet updateStatus(long userId, long resourcePrimKey, int status,
		...
		if (status == WorkflowConstants.STATUS_APPROVED) {
			...
			assetEntryLocalService.updateVisible(Talelet.class.getName(),
